...
extension CitySelectViewController: UICollectionViewDataSource,UICollectionViewDelegate, UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(collectionView: UICollectionView, didSelectItemAtIndexPath indexPath: NSIndexPath) {
        ...
        //存储当前选择
        user.setObject(currentCityName, forKey: SD_Current_SelectedCity)
        //选择后立即同步并返回
        if user.synchronize() {
            NSNotificationCenter.defaultCenter().postNotificationName(SD_CurrentCityChange_Notification, object: currentCityName)
