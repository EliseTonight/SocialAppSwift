...
class CitySelectViewController: UIViewController {
    ...
    lazy var domesticCity:NSMutableArray? = {
        let domesticCity = NSMutableArray(array: ["北京", "上海", "成都", "广州", "杭州", "西安", "重庆", "厦门", "台北"])
        return domesticCity
    }()
    lazy var overseaCity:NSMutableArray? = {
        let overseaCity = NSMutableArray(array: ["Strasbourg", "熊本县", "Paris", "东京", "Normandie", "London"])
        return overseaCity
    }()
...
extension CitySelectViewController: UICollectionViewDataSource,UICollectionViewDelegate, UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        //存储当前选择
        user.set(currentCityName, forKey: SD_Current_SelectedCity)
        //选择后立即同步并返回
        if user.synchronize() {
            NotificationCenter.default.post(name: Notification.Name(rawValue: SD_CurrentCityChange_Notification), object: currentCityName)
